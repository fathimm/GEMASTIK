...
dataset_path = '/path/to/local/dataset_modifikasi/'  # Ganti dengan path dataset lokal di Raspberry Pi

# Path untuk direktori output train, val, dan test
output_path = '/path/to/local/dataset_split'  # Ganti dengan path output yang diinginkan
...
PATH = '/path/to/local/resnet.pth'  # Ganti dengan path yang diinginkan untuk menyimpan model
...
print("Accuracy on Test Set: {:.2f}%".format(accuracy_score(all_labels, all_preds) * 100))
print("Classification Report:\n", classification_report(all_labels, all_preds, target_names=cabe_label))

cm = confusion_matrix(all_labels, all_preds)
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=cabe_label, yticklabels=cabe_label)
